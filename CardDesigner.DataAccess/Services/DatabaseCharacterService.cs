    ...
    public class DatabaseCharacterService : ICharacterService
    {
        ...
        public async Task<CharacterModel> UpdateCharacter(CharacterModel characterModel)
        {
            using CardDesignerDbContext dbContext = _dbContextFactory.CreateDbContext();
            {
                if (false)
                {
                    // This is causing DeckDescriptors to not be removed
                    CharacterEntity characterCardEntity = _mapper.Map<CharacterEntity>(characterModel);
                    CharacterEntity createdCharacterCardEntity = dbContext.Characters.Update(characterCardEntity).Entity;
